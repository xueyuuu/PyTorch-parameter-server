        ...
        for i, output in reversed(list(enumerate(self.output))):
            ...
            if i == (len(self.output) - 1):
                ...
                if not pd.isnull(tmp_grad):
                    ...
                    req_isend = communicator.Isend([grads, MPI.DOUBLE], dest=0,
                                                   tag=generate_tag(layer_tag=88 + channel_index, step_token=cur_step))
                ...
                if pd.isnull(self.full_modules[mod_avail_index].bias):
                    ...
                    if not pd.isnull(tmp_grad_weight):
                        ...
                        # req_isend = communicator.Isend([grads, MPI.DOUBLE], dest=0, tag=88+channel_index)
                        req_isend = communicator.Isend([grads, MPI.DOUBLE], dest=0,
                                                       tag=generate_tag(layer_tag=88 + channel_index,
                                                                        step_token=cur_step))
                        req_send_check.append(req_isend)
                        channel_index -= 1
